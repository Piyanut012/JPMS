        ...
        if(btn.equals(confirm)){
            ...
            if(x == 0){
                for(int p : id_pawn){
                    ...
                    currentFrame.Search(currentFrame.getCurrent_customer().getId());
                }
                dialog.dispose();
            }
    ...
    public void tableChanged(TableModelEvent e) {
        ...
        if (e.getType() == TableModelEvent.UPDATE && e.getColumn() == 0) {
            ...
            if (checked) {
                ...
                id_pawn.add(id);
                total += current_customer_allpawn.get(id).getValue() + (current_customer_allpawn.get(id).monthTime() * current_customer_allpawn.get(id).InterestPrice());
            }else{
                Integer id = (Integer) model.getValueAt(row, 1);
                id_pawn.remove(id);
                total -= current_customer_allpawn.get(id).getValue() + (current_customer_allpawn.get(id).monthTime() *current_customer_allpawn.get(id).InterestPrice());
